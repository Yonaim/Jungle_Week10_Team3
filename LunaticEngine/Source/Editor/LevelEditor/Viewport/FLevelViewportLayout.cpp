...
bool DrawCameraPopupScalarRow(const char *Id, const char *Label, float &Value, float Speed, float Min, float Max,
                              const char *Format)
{
    ImGui::PushID(Id);
    ...
    const float InputWidth = (std::min)(CameraPopupMaxScalarFieldWidth,
...
bool DrawToolbarIconButton(const char *Id, EToolbarIcon Icon, const char *FallbackLabel, float FallbackSize,
                           float MaxIconSize, ImU32 IconTint = IM_COL32_WHITE)
{
    ...
    return ImGui::ImageButton(Id, reinterpret_cast<ImTextureID>(IconSRV), IconSize, ImVec2(0.0f, 0.0f),
                              ImVec2(1.0f, 1.0f), ImVec4(0, 0, 0, 0), ImGui::ColorConvertU32ToFloat4(IconTint));
}

bool DrawToolbarIconLabelButton(const char *Id, EToolbarIcon Icon, const char *Label, float Width, float Height,
                                float FallbackSize, float MaxIconSize, ImU32 IconTint = IM_COL32_WHITE)
{
    ...
    {
        DrawList->AddImage(reinterpret_cast<ImTextureID>(IconSRV), ImVec2(IconX, IconY),
                           ImVec2(IconX + IconSize.x, IconY + IconSize.y), ImVec2(0, 0), ImVec2(1, 1), IconTint);
    }
...
bool DrawToolbarIconDropdownButton(const char *Id, EToolbarIcon Icon, float Width, float Height, float FallbackSize,
                                   float MaxIconSize, ImU32 IconTint = IM_COL32_WHITE)
{
    const bool bClicked = ImGui::Button(Id, ImVec2(Width, Height));
...
bool DrawSelectedToolbarIconDropdownButton(const char *Id, EToolbarIcon Icon, bool bSelected, float Width, float Height,
                                           float FallbackSize, float MaxIconSize,
                                           ImU32 SelectedTint = EditorAccentColor::ToU32())
{
...
void DrawToolbarDropdownArrowForLastItem()
{
    ...
    const ImVec2 Center(RectMax.x - ArrowRightPadding - ArrowWidth * 0.5f,
...
void DrawSnapPopupOptions(const char *Label, bool &bEnabled, float &Value, const float *Options, int32 OptionCount,
                          const char *Format)
{
    ImGui::PushID(Label);
    ImGui::PushStyleColor(ImGuiCol_Text, CameraPopupLabelColor);
...
void RenderSnapPopupContent(int32 SlotIndex, FEditorSettings &Settings, float FallbackSize, float MaxIconSize)
{
    ...
    const float LocationWidth =
        GetToolbarIconLabelButtonWidth(EToolbarIcon::TranslateSnap, "Location", FallbackSize, MaxIconSize);
    const float RotationWidth =
        GetToolbarIconLabelButtonWidth(EToolbarIcon::RotateSnap, "Rotation", FallbackSize, MaxIconSize);
    const float ScaleWidth =
        GetToolbarIconLabelButtonWidth(EToolbarIcon::ScaleSnap, "Scale", FallbackSize, MaxIconSize);

    auto DrawSnapTabButton = [&](const char *Id, EToolbarIcon Icon, const char *Label, ESnapPopupType Type,
                                 float Width) {
    ...
    DrawSnapTabButton("##SnapTabLocation", EToolbarIcon::TranslateSnap, "Location", ESnapPopupType::Location,
                      LocationWidth);
    ImGui::SameLine(0.0f, 8.0f);
    DrawSnapTabButton("##SnapTabRotation", EToolbarIcon::RotateSnap, "Rotation", ESnapPopupType::Rotation,
                      RotationWidth);
    ImGui::SameLine(0.0f, 8.0f);
    ...
    case ESnapPopupType::Location:
        DrawSnapPopupOptions("Location", Settings.bEnableTranslationSnap, Settings.TranslationSnapSize,
                             TranslationSnapSizes, 9, "%.0f");
        break;
    case ESnapPopupType::Rotation:
        DrawSnapPopupOptions("Rotation", Settings.bEnableRotationSnap, Settings.RotationSnapSize, RotationSnapSizes, 8,
                             "%.0f deg");
        break;
...
void RenderSnapToolbarButton(int32 SlotIndex, FEditorSettings &Settings, float Width, float FallbackSize,
                             float MaxIconSize)
{
    char ButtonId[64];
    char PopupId[64];
    ...
    Width =
        (std::max)(Width, GetToolbarIconDropdownButtonWidth(EToolbarIcon::TranslateSnap, FallbackSize, MaxIconSize));

    const bool bAnySnapEnabled =
        Settings.bEnableTranslationSnap || Settings.bEnableRotationSnap || Settings.bEnableScaleSnap;
    if (DrawSelectedToolbarIconDropdownButton(ButtonId, EToolbarIcon::TranslateSnap, bAnySnapEnabled, Width, 26.0f,
                                              FallbackSize, MaxIconSize))
    {
        if (GSnapPopupTab[SlotIndex] == ESnapPopupType::None)
        {
            GSnapPopupTab[SlotIndex] = ESnapPopupType::Location;
...
bool DrawSearchInputWithIcon(const char *Id, const char *Hint, char *Buffer, size_t BufferSize, float Width)
{
    ...
    if (ID3D11ShaderResourceView *SearchIcon =
            FResourceManager::Get()
                .FindLoadedTexture(FResourceManager::Get().ResolvePath(FName("Editor.Icon.Search")))
                .Get())
    {
...
bool FLevelViewportLayout::SubtreeContainsWindow(SWindow *Node, SWindow *TargetWindow) const
{
    ...
    return Splitter && (SubtreeContainsWindow(Splitter->GetSideLT(), TargetWindow) ||
...
void FLevelViewportLayout::BeginSplitToOnePaneTransition(int32 SlotIndex)
{
    ...
    if (!RootSplitter || SlotIndex < 0 || SlotIndex >= static_cast<int32>(LevelViewportClients.size()) ||
        SlotIndex >= MaxViewportSlots || !ViewportWindows[SlotIndex])
    {
        MaximizedOriginalSlotIndex = 0;
...
void FLevelViewportLayout::BeginOnePaneToSplitTransition(EViewportLayout TargetLayout)
{
    ...
    const int32 ExpandSourceSlot = (MaximizedOriginalSlotIndex >= 0 && MaximizedOriginalSlotIndex < TargetSlotCount)
...
void FLevelViewportLayout::SetLayout(EViewportLayout NewLayout)
{
    ...
    {
        constexpr ELevelViewportType DefaultTypes[] = {ELevelViewportType::Top, ELevelViewportType::Front,
...
void FLevelViewportLayout::ToggleViewportSplit(int32 SourceSlotIndex)
{
    ...
    else
    {
        const int32 SlotIndex =
            (SourceSlotIndex >= 0 && SourceSlotIndex < static_cast<int32>(LevelViewportClients.size()) &&
...
void FLevelViewportLayout::RenderViewportUI(float DeltaTime)
{
    ...
    EditorPanelTitleUtils::DrawSmallPanelCloseButton("    Viewport", FEditorSettings::Get().UI.bViewport,
                                                     "x##CloseViewport");
    if (!bIsOpen)
    {
        ImGui::End();
    ...
    if (ImGui::GetDragDropPayload())
    {
        ...
        if (ImGui::BeginDragDropTarget())
        {
            if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("ObjectContentItem"))
            {
                ...
                AStaticMeshActor *NewActor = Cast<AStaticMeshActor>(
                    FObjectFactory::Get().Create(AStaticMeshActor::StaticClass()->GetName(), Editor->GetWorld()));
    ...
    if (ContentSize.x > 0 && ContentSize.y > 0)
    {
        ...
        FRect ContentRect = {ContentPos.x, ContentPos.y + MainToolbarHeight, ContentSize.x,
...
void FLevelViewportLayout::RenderMainToolbar(float ToolbarLeft, float ToolbarTop)
{
    ...
    auto DrawGizmoIcon = [&](const char *Id, EToolbarIcon Icon, EGizmoMode TargetMode,
                             const char *FallbackLabel) -> bool {
        ...
        const bool bClicked =
            DrawToolbarIconButton(Id, Icon, FallbackLabel, ToolbarFallbackIconSize, ToolbarMaxIconSize);
        if (bSelected)
        {
    ...
    if (DrawToolbarIconButton("##SharedAddActorIcon", EToolbarIcon::AddActor, "Add", ToolbarFallbackIconSize,
                              ToolbarMaxIconSize))
    {
        ...
        for (int32 i = 0; i < ActiveSlotCount; ++i)
        {
            if (LevelViewportClients[i] == ActiveViewportClient)
            {
                ...
                if (ViewportWindows[i])
                {
                    ...
                    ContextMenuState.PendingSpawnPos = {ViewRect.X + ViewRect.Width * 0.5f,
    ...
    auto DrawSnapControl = [&](const char *Id, EToolbarIcon Icon, const char *FallbackLabel, bool &bEnabled,
    ...
    DrawSnapControl("RotateSnap", EToolbarIcon::RotateSnap, "Rotation Snap", Settings.bEnableRotationSnap,
                    Settings.RotationSnapSize, 0.001f);
    DrawSnapControl("ScaleSnap", EToolbarIcon::ScaleSnap, "Scale Snap", Settings.bEnableScaleSnap,
                    Settings.ScaleSnapSize, 0.001f);
...
void FLevelViewportLayout::RenderViewportToolbar(int32 SlotIndex)
{
    ...
    ImGuiWindowFlags OverlayFlags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings |
    ...
    ImGui::Begin(OverlayID, nullptr, OverlayFlags);
    {
        ...
        PushToolbarButtonStyle();
        {
            if (SlotIndex < static_cast<int32>(LevelViewportClients.size()))
            {
                ...
                auto DrawSelectedToolbarIcon = [&](const char *Id, EToolbarIcon Icon, bool bSelected,
                                                   const char *Tooltip) -> bool {
                    const ImU32 Tint = bSelected ? EditorAccentColor::ToU32() : IM_COL32_WHITE;
                    const bool bClicked = DrawToolbarIconButton(Id, Icon, Tooltip, PaneToolbarFallbackIconSize,
                                                                PaneToolbarMaxIconSize, Tint);
                    ShowItemTooltip(Tooltip);
                    return bClicked;
                };

                auto DrawDropdownToolbarButton = [&](const char *Id, EToolbarIcon Icon, const char *Label,
                                                     const char *Tooltip, float Width, bool bShowLabel) -> bool {
                    const bool bClicked =
                        !bShowLabel ? DrawToolbarIconDropdownButton(Id, Icon, Width, 26.0f, PaneToolbarFallbackIconSize,
                ...
                static const char *ViewportTypeNames[] = {"Perspective", "Top",   "Bottom", "Left",
                                                          "Right",       "Front", "Back",   "Free Ortho"};
                ...
                if (Gizmo)
                {
                    if (bShowTranslateButton)
                    {
                        ...
                        if (DrawSelectedToolbarIcon("##TranslateTool", EToolbarIcon::Translate,
                                                    Gizmo->GetMode() == EGizmoMode::Translate, "Translate"))
                        {
                            Gizmo->SetTranslateMode();
                        }
                    }
                    ...
                    {
                        BeginToolbarItem(EffectiveButtonSpacing);
                        if (DrawSelectedToolbarIcon("##RotateTool", EToolbarIcon::Rotate,
                                                    Gizmo->GetMode() == EGizmoMode::Rotate, "Rotate"))
                        {
                            Gizmo->SetRotateMode();
                        }
                    }
                    ...
                    {
                        BeginToolbarItem(EffectiveButtonSpacing);
                        if (DrawSelectedToolbarIcon("##ScaleTool", EToolbarIcon::Scale,
                                                    Gizmo->GetMode() == EGizmoMode::Scale, "Scale"))
                        {
                            Gizmo->SetScaleMode();
                        }
                    }
                ...
                if (bShowCoordButton)
                {
                    ...
                    if (DrawSelectedToolbarIcon("##CoordSystem",
                                                bWorldCoord ? EToolbarIcon::WorldSpace : EToolbarIcon::LocalSpace,
                ...
                {
                    BeginToolbarItem(EffectiveButtonSpacing);
                    RenderSnapToolbarButton(SlotIndex, Settings, bUseCompactToolbarLayout ? 30.0f : 36.0f,
                                            PaneToolbarFallbackIconSize, PaneToolbarMaxIconSize);
                }

                const float PopupButtonWidth =
                    PaneRect.Width >= 700.0f ? 46.0f : (bUseCompactToolbarLayout ? 30.0f : 36.0f);
                ...
                const float EffectiveViewportWidth =
                    bShowViewportLabel
                        ? GetToolbarIconLabelButtonWidth(GetViewportTypeToolbarIcon(Opts.ViewportType),
                                                         CurrentViewportTypeName, PaneToolbarFallbackIconSize,
                                                         PaneToolbarMaxIconSize)
                        : PopupButtonWidth;
                const float EffectiveViewModeWidth =
                    bShowViewModeButton
                        ? (bShowViewModeLabel ? GetToolbarIconLabelButtonWidth(
                                                    GetViewModeToolbarIcon(Opts.ViewMode), CurrentViewModeName,
                                                    PaneToolbarFallbackIconSize, PaneToolbarMaxIconSize)
                                              : PopupButtonWidth)
                        : 0.0f;
                ...
                if (bShowCameraMenu)
                {
                    ...
                    if (DrawDropdownToolbarButton("##CameraSettingsIcon", EToolbarIcon::CameraSettings, "Camera",
                                                  "Camera Settings", PopupButtonWidth, false))
                    {
                ...
                BeginRightToolbarItem(RightButtonSpacing);
                if (DrawDropdownToolbarButton("##ViewportTypeIcon", GetViewportTypeToolbarIcon(Opts.ViewportType),
                                              CurrentViewportTypeName, "Viewport Type", EffectiveViewportWidth,
                                              bShowViewportLabel))
                {
                ...
                if (ImGui::BeginPopup(ViewportPopupID))
                {
                    auto DrawViewportTypeOptions = [&](const char *SectionLabel, int32 StartIndex, int32 EndIndex) {
                        ...
                        for (int32 TypeIndex = StartIndex; TypeIndex < EndIndex; ++TypeIndex)
                        {
                            ...
                            {
                                DrawList->AddImage(reinterpret_cast<ImTextureID>(IconSRV),
                ...
                {
                    BeginRightToolbarItem(RightButtonSpacing);
                    if (DrawDropdownToolbarButton("##ViewModeIcon", GetViewModeToolbarIcon(Opts.ViewMode),
                                                  CurrentViewModeName, "View Mode", EffectiveViewModeWidth,
                                                  bShowViewModeLabel))
                    {
                ...
                {
                    BeginRightToolbarItem(RightButtonSpacing);
                    if (DrawDropdownToolbarButton("##SettingsIcon", EToolbarIcon::ShowFlag, "Show", "Show",
                ...
                {
                    BeginRightToolbarItem(RightButtonSpacing);
                    if (DrawDropdownToolbarButton("##Layout", EToolbarIcon::Menu, "Layout", "Viewport Layout",
                                                  PopupButtonWidth, false))
                    {
                ...
                if (bShowLayoutButton && ImGui::BeginPopup(LayoutPopupID))
                {
                    ...
                    for (int32 i = 0; i < LayoutCount; ++i)
                    {
                        ...
                        if (LayoutIcons[i])
                        {
                            bClicked = ImGui::ImageButton("##icon", reinterpret_cast<ImTextureID>(LayoutIcons[i]),
                ...
                if (bShowToggleButton)
                {
                    ...
                    ShowItemTooltip((CurrentLayout == EViewportLayout::OnePane) ? "Split Viewports"
                                                                                : "Merge Viewports");
                    if (bIsTransitioning)
                    {
                        ImGui::EndDisabled();
        ...
        if (DrawToolbarIconButton("##Layout", EToolbarIcon::Menu, "Layout", PaneToolbarFallbackIconSize,
                                  PaneToolbarMaxIconSize))
        {
            ImGui::OpenPopup(PopupID);
        }
        ...
        if (LayoutIcons[ToggleIdx])
        {
            if (ImGui::ImageButton("##toggle", (ImTextureID)LayoutIcons[ToggleIdx],
                                   ImVec2(ToggleIconSize, ToggleIconSize)))
            {
                ToggleViewportSplit(SlotIndex);
            }
        ...
        if (SlotIndex < static_cast<int32>(LevelViewportClients.size()))
        {
            ...
            auto DrawSelectedToolbarIcon = [&](const char *Id, EToolbarIcon Icon, bool bSelected,
                ...
                const bool bClicked =
                    DrawToolbarIconButton(Id, Icon, Tooltip, PaneToolbarFallbackIconSize, PaneToolbarMaxIconSize);
                if (bSelected)
                {
                    ImGui::PopStyleColor(3);
                }
            ...
            if (DrawSelectedToolbarIcon("##PlaceActor", EToolbarIcon::AddActor, false, "Place Actor"))
            {
                ...
                ContextMenuState.PendingSpawnPos = {PaneRect.X + PaneRect.Width * 0.5f,
            ...
            if (Gizmo)
            {
                ...
                ImGui::SameLine(0.0f, PaneToolbarButtonSpacing);
                if (DrawSelectedToolbarIcon("##ScaleTool", EToolbarIcon::Scale, Gizmo->GetMode() == EGizmoMode::Scale,
                                            "Scale"))
                {
                    Gizmo->SetScaleMode();
                }
            ...
            ImGui::SameLine(0.0f, 10.0f);
            if (DrawSelectedToolbarIcon("##CoordSystem",
                                        bWorldCoord ? EToolbarIcon::WorldSpace : EToolbarIcon::LocalSpace, bWorldCoord,
            ...
            const float RightGroupWidth = ViewModeButtonWidth + ViewportButtonWidth + CameraButtonWidth +
                                          ShowButtonWidth + PaneToolbarButtonSpacing * 3.0f;
            ...
            if (DrawToolbarIconLabelButton("##ViewModeIcon", GetViewModeToolbarIcon(Opts.ViewMode), CurrentViewModeName,
                                           ViewModeButtonWidth, 26.0f, PaneToolbarFallbackIconSize,
                                           PaneToolbarMaxIconSize))
            {
                ImGui::OpenPopup(ViewModePopupID);
            }
            ...
            if (DrawToolbarIconLabelButton("##ViewportTypeIcon", GetViewportTypeToolbarIcon(Opts.ViewportType),
                                           CurrentViewportTypeName, ViewportButtonWidth, 26.0f,
                                           PaneToolbarFallbackIconSize, PaneToolbarMaxIconSize))
            {
                ImGui::OpenPopup(ViewportPopupID);
            }
            ...
            if (DrawToolbarIconButton("##CameraSettingsIcon", EToolbarIcon::CameraSettings, "Camera",
                                      PaneToolbarFallbackIconSize, PaneToolbarMaxIconSize))
            {
                ImGui::OpenPopup(CameraPopupID);
            }
...
void FLevelViewportLayout::HandleViewportContextMenuInput(const FPoint &MousePos)
{
    ...
    for (int32 i = 0; i < ActiveSlotCount; ++i)
    {
        ...
        const bool bClickCandidate = bReleasedOverSameSlot &&
...
bool FLevelViewportLayout::TryComputePlacementLocation(int32 SlotIndex, const FPoint &ClientPos,
                                                       FVector &OutLocation) const
{
    ...
    const float VPWidth =
        ViewportClient->GetViewport() ? static_cast<float>(ViewportClient->GetViewport()->GetWidth()) : ViewRect.Width;
